    ...
    const transcribeAudio = async (audioFile: File, selectedMode: Mode) => {
        try {
            ...
            if (selectedMode === 'song') {
                promptText = "Transcribe this worship song. Group words into natural lyric lines/stanzas in the 'lines' array. Set type to 'lyric'. Do NOT merge stanzas into big blocks. Provide precise timestamps for every word.";
            } else {
                promptText = `
...
Group words into these structural lines with precise timestamps for every word.
    ...
    const handleExportToPowerPoint = async () => {
        ...
        try {
            ...
            // Set RTL if detected, though PptxGenJS support for RTL is limited on slide level, setting it on text items helps
            ...
            if (mode === 'song') {
                ...
                // SPEECH/BIBLE MODE
                // Respect structural breaks. Title = New Slide. Chapter = New Slide.
                // Verses = Group by ~3 lines.
            ...
            // Generate as PPSX (PowerPoint Show)
        ...
        return (
            <div className={`space-y-4 ${fontFamily}`} dir={direction}>
                {transcriptData.lines.map((line, lineIndex) => {
                    ...
                    // Specific styles based on LineType
                    ...
                    return (
                        ...
                        >
                            {/* Verse Label Badge */}
                            {isVerse && line.label && (
                                <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-bold mr-2 mb-1 align-middle ${isLineActive ? 'bg-teal-500 text-white' : 'bg-gray-700 text-gray-400'}`}>
                                    {line.label}
                                </span>
                            )}
